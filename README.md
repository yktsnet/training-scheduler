...
- **ゆるやかな識別（アニマル・ログイン）**: パスワード等による厳格な認証ではなく、動物の絵文字を選ぶだけのシンプルなログインを採用。チーム内の信頼関係を前提とした、遊び心のあるアカウント管理です。同一アニマルの重複利用時に混同しないよう、登録時に任意のイニシャル（例: "YT"）を設定して識別可能です。

### Demo Mode (デモモード)

本アプリにはデモ用の「デモモード」が備わっています（デフォルト有効）。
- **データ自動リセット**: デモ公開時の改ざんを防ぐため、30分ごとにDBを初期ダミーデータ（🐶 ユーザー、計画、日報、進捗）へ自動復元します。
- **自動ログイン**: 初回アクセス時のアニマル選択をバイパスし、即座にダミー（🐶）として機能を体験できます。
- **環境設定による無効化**:
  実運用時は環境変数 `DEMO_MODE=false` および管理者ログイン用のパスワード `ADMIN_PASSWORD` を指定して起動します。
  ```bash
  DEMO_MODE=false ADMIN_PASSWORD=your_secure_password ./backend/training-app
  ```

### Admin & Mentor Operations (管理者・メンター機能)

- **研修メニューの編集**:
  - 管理者パネル（パスワード認証）から、研修メニューの追加・編集・削除が行えます（変更は `menu_config.json` に自動同期されます）。
- **進捗の確認（見守り）**:
  - ダッシュボード（Overview）から、受講生全員の研修開始日、進捗状況（手応え）、ステータスメモ、過去の日報を一覧で閲覧できます。
...
### In Scope
- 各自のアニマル（絵文字）とイニシャル（例: "YT"）による簡易ログイン

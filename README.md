...
本アプリにはデモ用の「デモモード」（デフォルト）と、実運用のための「通常モード」が備わっています。

* **デモモード (デフォルト / 環境変数未指定時)**
...
* **通常モード (実運用 / デモ無効時)**
  * **有効化**: 環境変数 `DEMO_MODE=false` を指定して起動します。
  * **クリーン起動**: 自動リセットやダミーデータの自動投入は行われません。アクセス時はアニマル選択画面から始まり、各自がアニマルを新規作成して研修を開始します。
  * **起動方法と環境設定**:
    プロジェクトルートの `.env.example` をコピーして `.env` ファイルを作成して設定するか、`systemd` のサービスファイル等で環境変数を指定します。
    ```bash
    # デモモードを無効化し、管理者パスワードを設定して起動
